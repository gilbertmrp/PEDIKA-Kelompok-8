...
import 'package:violence_app/model/wilayah/pelaporan/districts.dart';
import 'package:violence_app/model/wilayah/pelaporan/provincies.dart';
...
class LocationProvider with ChangeNotifier {
  ...
  Future<void> fetchProvinces() async {
    final response = await http.get(Uri.parse('${Config.AREA_API}${Config.PROVINCES}.json'));
    if (response.statusCode == 200) {
      List<dynamic> provincesJson = json.decode(response.body);
      _provinces = provincesJson.map((json) => Provincies.fromJson(json)).toList();
      notifyListeners();
    } else {
  ...
  Future<void> fetchCities(String provinceId) async {
    final response = await http.get(Uri.parse('${Config.AREA_API}${Config.CITIES}/$provinceId.json'));
    if (response.statusCode == 200) {
  ...
  Future<void> fetchDistricts(String cityId) async {
    final response = await http.get(Uri.parse('${Config.AREA_API}${Config.DISTRICTS}/$cityId.json'));
    if (response.statusCode == 200) {
      List<dynamic> districtsJson = json.decode(response.body);
      _districts = districtsJson.map((json) => Districts.fromJson(json)).toList();
      notifyListeners();
    } else {
  ...
  Future<void> fetchSubDistricts(String districtId) async {
    final response = await http.get(Uri.parse('${Config.AREA_API}${Config.SUB_DISTRICTS}/$districtId.json'));
    if (response.statusCode == 200) {
      List<dynamic> subDistrictsJson = json.decode(response.body);
      _subDistricts = subDistrictsJson.map((json) => SubDistricts.fromJson(json)).toList();
      notifyListeners();
    } else {
